...
export class IntermecPrinter extends IntermecPrinterCommon {
	...
	public printer: com.ticnat.intermecprinter.PrinterConnector;
	...
	public connect(address: string) {
		...
		try {
			this.printer = new com.ticnat.intermecprinter.PrinterConnector(Application.android.foregroundActivity, this.address, 'PR3');
		} catch (e) {
			console.dir(e);
		}
	}
	...
	public printTextFile(file) {
		...
		file.readText().then((res) => {
			this.printer.printText(res, false, false, false);
		});
	}
	// TODO: Still has a problem with bmp print ... :(
	public printImg(bitmap) {
		try {
			this.printer.printBmp(bitmap);
		} catch (e) {
			console.log(e);
		}
	}
	...
	public print(string: string) {
		...
		if (string == null) return;
			...
			try {
				...
				// // let arabic = com.intermec.printer.intermecCmd.setCodeTable(com.intermec.printer.intermecCmd.MCU_RX, com.intermec.printer.intermecCmd.CT_DBCS, com.intermec.printer.intermecCmd.FONT_SMALL);
				// // if (arabic != null) {
				// //     // console.log(arabic.length);
				// //     byteStream.write(arabic);
				// // }
				// if (text != null) byteStream.write(text);
				// byteStream.write(com.intermec.printer.intermecCmd.printData());
				// this.sendData(com.intermec.printer.intermecCmd.initPrinter());
				// this.sendData(<any>byteStream.toByteArray());
				this.printer.printText(string, false, false, false);
			} catch (e) {
				console.log(e);
			}
		}
