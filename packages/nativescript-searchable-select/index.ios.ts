import { Button, FlexboxLayout, Frame, GridLayout, GridUnitType, isAndroid, ItemSpec, Label, ListView, Observable, ObservableArray, Page, SearchBar, StackLayout } from '@nativescript/core';
...
export class SearchableSelect extends SearchableSelectCommon {
	...
	private doneSelect() {
		...
		self.closeModal();
		setTimeout(function () {
			if (self.searchBar) self.searchBar.text = '';
		}, 500);
	}
	...
	private Modal() {
		...
		listView.on('itemTap', function (args) {
			if (self.multiple == false && args.view.className == 'item filter-select-selected') return 0;
			if (self.multiple == false) self.selected_items.splice(0, self.selected_items.length);
			if (args.view.className != 'item filter-select-selected') {
				args.view.className = 'item filter-select-selected';
				if (self.selected_flag) args.view.bindingContext[self.selected_flag] = true;
			} else {
				args.view.className = 'item';
				if (self.selected_flag) args.view.bindingContext[self.selected_flag] = false;
			}
