...
export class WoosimPrinter extends WoosimPrinterCommon {
	...
	public async connect(address: string) {
		console.log(this.ptrConn.getState());
	...
	private sendData(data: any[]) {
		...
		if (!this.isConnected()) {
			...
			if (this.debug) console.log('not Connected');
			return;
		}
	...
	public printImg(bitmap, setExtraPaddingAfterPrint = true, startX = 0, startY = 0) {
		try {
			...
			if (setExtraPaddingAfterPrint) this.print('\n\n');
		} catch (e) {
			console.log(e);
		}
	}
	...
	public print(string: string) {
		...
		if (string == null) return;
			...
			try {
				...
				let byteStream = new java.io.ByteArrayOutputStream();
				// let arabic = com.woosim.printer.WoosimCmd.setCodeTable(com.woosim.printer.WoosimCmd.MCU_RX, com.woosim.printer.WoosimCmd.CT_DBCS, com.woosim.printer.WoosimCmd.FONT_SMALL);
				// if (arabic != null) {
